...
//
// Since .NET was launched, more and more notifications have been added
// by versioning the interface up to ICorProfilerCallback9.
//
class CorProfiler : public ICorProfilerCallback9
{
private:
...
public:
    ...
    static DWORD STDMETHODCALLTYPE GetThreadId();

protected:
    BOOL GetClassAndMethodFromFunctionId(FunctionID uiFunctionId, LPWSTR szClass, UINT uiClassLen, LPWSTR szMethod, UINT uiMethodLen);
    BOOL GetClassFromObjectId(ObjectID  uiObjectId, LPWSTR szClass, UINT uiClassLen);
    HRESULT InspectCLRVersion(IUnknown* corProfilerInfoUnk);
};